<x-admin-layout>
    <div class="pb-6 pt-1">
        <p class="font-bold md:text-5xl text-2xl">Halaman Ubah Data Siswa</p>
        <p class="text-gray-500 py-1 text-xs md:text-lg">Silahkan Ubah Data Siswa yang Salah Menjadi Benar</p>
    </div>
    ...
    <div class="w-full max-w-xll p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
        <form method="POST" action="{{ route('siswa.update', $siswa->id) }}">
            ...
            <div class="mt-4">
                ...
                <x-text-input id="nis" class="block mt-1 w-full" type="number" name="nis" value="{{ $siswa->nis }}" required autofocus autocomplete="nis" />
                <x-input-error :messages="$errors->get('nis')" class="mt-2" />
            </div>
            {{-- Nama siswa --}}
            <div class="mt-4">
                <x-input-label for="nama_siswa" :value="__('Nama Siswa')" />
                <x-text-input id="text-input" class="block mt-1 w-full" type="text" name="nama_siswa" value="{{ $siswa->nama_siswa }}" required autofocus autocomplete="nama_siswa" />
