<x-admin-layout>
    ...
    <div class="w-full max-w-xll p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
        <form method="POST" action="{{ route('ubahPelajaran', $pelajaran->id) }}">
            @csrf
            @method('patch')
            {{-- Nama Pelajaran --}}
            <div class="mt-4">
                <x-input-label for="nama_pelajaran" :value="__('Nama Pelajaran')" />
                <x-text-input id="nama_pelajaran" class="block mt-1 w-full" type="text" name="nama_pelajaran" value="{{ $pelajaran->nama_pelajaran }}" required autofocus autocomplete="nama_pelajaran" />
